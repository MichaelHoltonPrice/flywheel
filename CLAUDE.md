...
- **Block executors** (`flywheel/executor.py`): The `BlockExecutor` protocol decouples block execution from Docker. Two implementations: `ContainerExecutor` (Docker container) and `ProcessExecutor` (local subprocess for trusted host-side processes like game servers). Both return an `ExecutionHandle` with `stop()`, `wait()`, `is_alive()`. Phase 5 of the block-execution refactor removed the `RecordExecutor` and the `__record__` sentinel; logical block executions that have no container body now use `runner: lifecycle` and are driven entirely through the execution-channel lifecycle API.
- **Execution channel** (`flywheel/execution_channel.py`): HTTP service hosting (1) the **lifecycle API** — `POST /execution/begin` and `POST /execution/end/{id}` — used by MCP tools inside agent containers via `flywheel.tool_block.BlockChannelClient`, and (2) the legacy invoke endpoint that runs `runner: container` blocks against an artifact directory. Fires typed `ExecutionEvent` callbacks. The legacy `mode=record` path was removed in Phase 5.
- **Logical block executions**: A side-effecting MCP tool can declare itself the body of a block (`runner: lifecycle`) and bind to that block via a tool-block manifest (`workforce/<server>_tool_blocks.yaml`). The tool wraps its work in `BlockChannelClient.begin(...)`, which calls the lifecycle API to resolve declared inputs (including `derive_from` rollups), reserves an execution row, lets the tool body run, and atomically registers outputs on `end`.
- **Block templates and registry**: Templates declare blocks by string reference (e.g., `blocks: [predict, game_step]`). Block bodies live in `workforce/blocks/<name>.yaml` and are loaded into a `BlockRegistry`. Inline-dict block definitions in templates were removed in Phase 5.
