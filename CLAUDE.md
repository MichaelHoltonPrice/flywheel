...
- **Local block recorder** (`flywheel/local_block.py`): Host-side, in-process write surface for `runner: lifecycle` blocks invoked during a full-stop handoff. Stages each declared input into a per-mount tempdir (snapshot for incremental inputs), allocates an execution id and a per-execution output tempdir exposing `ctx.output_dir(name)`, lets the block body run, and registers each declared output from disk before writing a single `succeeded`/`failed` row. There is no `running` row and no HTTP. Post-execution checks fire after the row is durable; halt directives are queued for the handoff loop to drain.
