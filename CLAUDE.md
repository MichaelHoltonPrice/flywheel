...
- **Block executors** (`flywheel/executor.py`): The `BlockExecutor` protocol decouples block execution from Docker. Two implementations: `ContainerExecutor` (Docker container) and `ProcessExecutor` (local subprocess for trusted host-side processes like game servers). Both return an `ExecutionHandle` with `stop()`, `wait()`, `is_alive()`. Phase 5 of the block-execution refactor removed the `RecordExecutor` and the `__record__` sentinel; logical block executions that have no container body now use `runner: lifecycle` and are recorded by `flywheel.local_block.LocalBlockRecorder`.
- **Local block recorder** (`flywheel/local_block.py`): Host-side, in-process write surface for `runner: lifecycle` blocks invoked during a full-stop handoff. Resolves declared inputs (including `derive_from` rollups), allocates an execution id, lets the block body run, and writes the resulting `succeeded`/`failed` row plus output artifacts atomically. There is no `running` row and no HTTP. Post-execution checks fire after the row is durable; halt directives are queued for the handoff loop to drain.
