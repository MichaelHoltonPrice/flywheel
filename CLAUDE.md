...
- **Agent loop** (`flywheel/agent_loop.py`): Flywheel-owned orchestration loop. Projects provide `decide(state) -> Action` and `build_prompt(action, state) -> str` hooks. The loop manages round counting, session resume, circuit breaker (consecutive auth/rate-limit failures), and lifecycle events. Actions: `Continue`, `SpawnGroup`, `Stop`, `Finished`. See `docs/architecture.md` "Agent loop".
